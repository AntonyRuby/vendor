...
class CampaignWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () => Get.toNamed(
        RouteHelper.getCampaignDetailsRoute(campaignModel.id ?? 0),
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(
                color: Colors.grey[Get.isDarkMode ? 700 : 300] ?? Colors.red,
                spreadRadius: 1,
                blurRadius: 5)
          ],
        ),
                  ...
                  InkWell(
                    onTap: () {
                      Get.dialog(ConfirmationDialog(
                        icon: Images.warning,
                        description: campaignModel.isJoined ?? false
                            ? 'are_you_sure_to_leave'.tr
                            : 'are_you_sure_to_join'.tr,
                        onYesPressed: () {
                          if (campaignModel.isJoined ?? false) {
                            Get.find<CampaignController>()
                                .leaveCampaign(campaignModel.id ?? 0, false);
                          } else {
                            Get.find<CampaignController>()
                                .joinCampaign(campaignModel.id ?? 0, false);
                          }
                        },
                      ));
                    },
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: campaignModel.isJoined ?? false
