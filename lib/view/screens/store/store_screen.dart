...
class _StoreScreenState extends State<StoreScreen>
  ...
  void initState() {
    ...
    Get.find<StoreController>().getStoreReviewList(
        Get.find<AuthController>().profileModel.stores?.first.id ?? 0);
  }
    ...
    return GetBuilder<StoreController>(builder: (storeController) {
      return GetBuilder<AuthController>(builder: (authController) {
        Store? _store = authController.profileModel != null
            ? authController.profileModel.stores?.first
            : null;
                    ...
                    SliverToBoxAdapter(
                      ...
                      child: Column(children: [
                        ...
                        Row(children: [
                          ...
                          Text(
                            (_store.avgRating ?? 0).toStringAsFixed(1),
                            style: robotoRegular.copyWith(
                                fontSize: Dimensions.FONT_SIZE_SMALL),
                          ),
                          SizedBox(width: Dimensions.PADDING_SIZE_SMALL),
                                    ...
                                    children: [
                                      Text(
                                        _store.discount?.discountType ==
                                            ...
                                            : '${PriceConverter.convertPrice(_store.discount?.discount ?? 0)} ${'off'.tr}',
                                        style: robotoMedium.copyWith(
                                            fontSize:
                                      ...
                                      Text(
                                        _store.discount?.discountType ==
                                            ...
                                            : '${'enjoy'.tr} ${PriceConverter.convertPrice(_store.discount?.discount ?? 0)}'
                                      ...
                                      _store.discount?.minPurchase != 0
                                          ? Text(
                                              '[ ${'minimum_purchase'.tr}: ${PriceConverter.convertPrice(_store.discount?.minPurchase ?? 0)} ]',
                                              style: robotoRegular.copyWith(
                                                  fontSize: Dimensions
