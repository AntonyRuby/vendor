...
class StoreController extends GetxController implements GetxService {
  ...
  Future<void> getItemList(String offset, String type) async {
    ...
    if (!_offsetList.contains(offset)) {
      ...
      if (response.statusCode == 200) {
        ...
        _pageSize = ItemModel.fromJson(response.body).totalSize ?? 0;
        _isLoading = false;
        update();
      } else {
  ...
  void getAttributeList(Item item) async {
    ...
    if (response.statusCode == 200) {
      ...
      response.body.forEach((attribute) {
        bool _active =
            item.attributes!.contains(Attr.fromJson(attribute).id ?? 0);
        List<String> _options = [];
        if (_active) {
          _options.addAll(item.choiceOptions![
              item.attributes?.indexOf(Attr.fromJson(attribute).id ?? 0) ??
                  0] as Iterable<String>);
        }
      ...
      if (item.addOns != null) {
        for (int index = 0; index < (item.addOns?.length ?? 0); index++) {
          setSelectedAddonIndex(
              _addonsIds.indexOf((item.addOns?[index].id ?? 0)), false);
        }
      }
    }
  ...
  void toggleAttribute(int index, Item product) {
    _attributeList[index].active = !(_attributeList[index].active ?? false);
  ...
  Future<void> updateStore(
    ...
    Response response = await storeRepo.updateStore(
        store, _rawLogo!, _rawCover!, min, max, type);
    if (response.statusCode == 200) {
      Get.back();
  ...
  Future<void> addItem(Item item, bool isAdd) async {
    ...
    if (_variantTypeList.length > 0) {
      ...
      _attributeList.forEach((attributeModel) {
        if (attributeModel.active ?? false) {
          _idList.add(attributeModel.attribute.id ?? 0);
    ...
    for (int i = 0; i < _length; i++) {
      ...
      for (Variation variation in (item.variations ?? [])) {
        if (variation.type == _value) {
          _price = variation.price ?? 0;
          _stock = variation.stock ?? 0;
          break;
        }
      }
