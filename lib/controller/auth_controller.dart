...
class AuthController extends GetxController implements GetxService {
  ...
  XFile? _pickedLogo;
  XFile? _pickedCover;
  LatLng? _restaurantLocation = LatLng(0, 0);
  ...
  LatLng get restaurantLocation => _restaurantLocation ?? LatLng(0, 0);
  ...
  Future<void> getProfile() async {
    ...
    if (response.statusCode == 200) {
      ...
      Get.find<SplashController>().setModule(
        _profileModel.stores?.first.module?.id ?? 0,
        _profileModel.stores?.first.module?.moduleType ?? '',
      );
      authRepo.updateHeader(
        _profileModel.stores?.first.module?.id ?? 0,
      );
    } else {
  ...
  Future<bool> updateUserInfo(
    ...
    Response response = await authRepo.updateProfile(
        updateUserModel, _pickedFile ?? XFile(''), token);
    _isLoading = false;
    bool _isSuccess;
    if (response.statusCode == 200) {
  ...
  Future<bool> clearSharedData() async {
    Get.find<SplashController>().setModule(0, '');
  ...
  void setLocation(LatLng location) async {
    ...
    if ((_response.isSuccess ?? false) && _response.zoneIds.length > 0) {
  ...
  Future<Position> setSuggestedLocation(
    ...
    if (response.statusCode == 200) {
      ...
      if (_placeDetails.status == 'OK') {
        _latLng = LatLng(
          _placeDetails.result?.geometry?.location?.lat ?? 0,
          _placeDetails.result?.geometry?.location?.lng ?? 0,
        );
      }
    }
